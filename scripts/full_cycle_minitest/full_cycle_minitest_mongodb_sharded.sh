#!/bin/bash
# showcases the ftsb 3 phases for MongoDB with a sharded collection
...
# insert benchmark
$GOPATH/bin/tsbs_load_mongo --db-name=benchmark --batch-size=100 --workers=1 --document-per-event=true --timeseries-collection=true --collection-sharded=true --retryable-writes=false --random-field-order=false --file=/tmp/bulk_data/mongo_data --results-file="mongo_load_results.json"
